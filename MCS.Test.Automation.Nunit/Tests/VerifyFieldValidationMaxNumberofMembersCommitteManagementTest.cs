// <copyright file="VerifyFieldValidationMaxNumberofMembersCommitteManagementTest.cs" company="PlaceholderCompany">
...
namespace MCS.Test.Automation.Tests.NUnit.Tests
{
    ...
    [TestFixture]
    public class VerifyFieldValidationMaxNumberofMembersCommitteManagementTest : ProjectTestBase
    {
        [Test]
        [Category("Regression")]
        [TestCaseDescription(Id = "MCS2-2008", Name = "Rules and Exceptions - Edit Committee Type Details - Field Validation - Max Number of Members Permitted")]
        [TestCaseSource(typeof(TestData), "GetDetailsFromXML", new object[] { "FieldValidionMaxNumbersCommiteeType" })]
        public void VerifyFieldValidationMaxNumberofMembersCommitteManagement(Dictionary<string, string> parameters)
        {
            ...
            manageCommiteeTypePage.IsLimitedPersonsSelected();
            manageCommiteeTypePage.IsAddNewCommitteeTypeSaveButtonClickable();
            string errorMessage = parameters["errorMessage"].Trim();
            manageCommiteeTypePage.IserrorMsgDisplayedFormandatoryInput(errorMessage);
            manageCommiteeTypePage.IsUnlimitedAttributeDisplayed("radio");
            manageCommiteeTypePage.IsLimitedAttributeDisplayed("radio");
            string unlimitedRadioLabel = parameters["unlimitedRadioLabel"].Trim();
            manageCommiteeTypePage.IsUnlimitedLabelDisplayed(unlimitedRadioLabel);
            string limitedRadioLabel = parameters["limitedRadioLabel"].Trim();
            manageCommiteeTypePage.IsLimitedLabelDisplayed(limitedRadioLabel);
        }
    }
}